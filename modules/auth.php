<?php
	...
	class Auth_Module extends Module_Support_Auth
	{
		...
		public function crypt($password, $salt = null)
		{
			...
			} else if (!$this->_hash_support($salt[1])) {
		...
		private function _hash_supported($version)
		{
			...
			return defined($hash) && constant($hash) == 1;
		}
		...
		public function change_cpassword($cpassword, $user = null, $domain = null)
		{
			...
			if ($this->permission_level & PRIVILEGE_SITE) {
				...
				if (!isset($users[$user])) return error($user . ": user not found");
			}
		...
		private function _change_cpassword_raw($cpassword, $user = null, $domain = null)
		{
			...
			} else if (!in_array($cpassword[1], array("1", "2", "5", "6"))) {
				/**
		...
		{
			if (!IS_CLI) return $this->query('auth_is_inactive');
		 ...
		 *
		 * @param integer $limit limit results retrieved to N results
		 *
		 * @return mixed
		 *
		 */
		...
		public function change_domain($domain)
		{
			...
			} else if (!preg_match(Regex::DOMAIN, $domain)) {
				return error("`%s': invalid domain", $domain);
			} else if (!is_debug() && $this->dns_domain_hosted($domain, true)) {
			...
			} else if ($this->aliases_shared_domain_exists($domain)) {
		...
		private function _username_unique($user)
		{
			...
			$db = $this->_connect_db();
			if (!$db) return error("cannot connect to db");
