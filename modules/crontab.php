<?php
	...
	class Crontab_Module extends Module_Skeleton {
		...
		public function list_jobs($user = null) {
			...
			if (!file_exists($spool)) return array();
			$fp = fopen($spool,'r');
			$cronjobs = array();
			while (!feof($fp)) {
				$line = fgets($fp);
		...
		public function disable_job($min, $hour, $dom, $month,
		                                $dow, $cmd, $user = null) {
			if (!IS_CLI) {
		...
		private function _setCronContents($contents, $user) {
			...
			if (!$pwd) return error("getpwnam() failed for user `%s'", $user);
		...
		public function add_cronjob($min, $hour, $dom, $month,
		                            $dow, $cmd, $user = null)
		{
		 ...
		 * @return bool
		 */
		public function add_job($min, $hour, $dom, $month,
									 $dow, $cmd, $user = null) {
			if (!IS_CLI) {
		...
		public function delete_cronjob($min, $hour, $dom, $month,
		                               $dow, $cmd, $user = null) {
		 ...
		 *
		 */
		public function delete_job($min, $hour, $dom, $month,
									 $dow, $cmd, $user = null){
			if (!IS_CLI)
				return $this->query("crontab_delete_job", $min, $hour,
					$dom, $month, $dow, $cmd, $user);
            
            if (!$this->enabled()) return error("crond is not enabled");
		...
		{
		    if (!IS_CLI) return $this->query('crontab_permit_user', $user);
		...
		{
		    if (!IS_CLI) return $this->query('crontab_deny_user', $user);
        ...
        public function list_users()
        {
            ...
            if (!file_exists($dir)) return $users;
            $dh = opendir($dir);
            while (false !== ($file = readdir($dh)) )
            {
                if ($file == "." || $file == "..") { continue; }
                else if (substr($file,0,4) == "tmp.") { continue; }
                $users[] = $file;
            }
            closedir($dh);
            return $users;
		...
		public function _edit_user($user, $usernew) {
            ...
            if (!$this->get_service_value('ssh','enabled')) {
                return true;
            } else if (!$this->user_permitted($user)) {
                return true;
            }
