<?php
	...
	class Demo_Module extends Module_Skeleton {
		...
		public function test_tail($cnt = 20, $timing = 0.5) {
			if (!is_debug()) return error("test_tail may not be used in production");
			if (!IS_CLI) return $this->query("demo_test_tail", $cnt, $timing);
		...
		public function test_tail2($cnt = 20, $timing = 0.5) {
			if (!is_debug()) return error("test_tail may not be used in production");
			if (!IS_CLI) return $this->query("demo_test_tail2", $cnt, $timing);
			$p = new Util_Process();
