<?php
	...
	class File_Module extends Module_Skeleton {
		...
		const UPLOAD_UID = WS_UID; // nobody from /etc/passwd
		...
		public function __wakeup() { $this->cached = Cache_User::spawn(); }
		/**
		 ...
		 * interfaces.
		 */
		public function get_registered_extensions() { return self::$registered_extensions; }
			...
			for ($i = 0, $nsource = sizeof($source); $i < $nsource; $i++) {
				...
				for ($j = 0, $nfiles = sizeof($files); $j < $nfiles; $j++) {
					...
					if ($optimized) {
						...
						if (!$fstat['can_read']) {
							$files_copied=0;
							error("cannot read `$local_file'"); continue;
						}
					}
					...
					if ($fstat['file_type'] != 'dir') {
						...
						if ($mkdir && !$this->create_directory($newdest,
							$fstat['permissions'],false)) continue;
        ...
        public function set_acls($file, $user, $permission = null, $xtra = array()) {
	        ...
	        if (!$sfiles) { return error("no files to adjust!"); }
