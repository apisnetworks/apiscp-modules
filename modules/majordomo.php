<?php
	...
	class Majordomo_Module extends Module_Skeleton {
		...
		public function create_mailing_list($list, $password, $email = NULL, $domain = NULL) {
			...
			else if (file_exists($this->domain_fs_path().'/var/lib/majordomo/config/'.$list))
				return error("Mailing list already exists");
			else if (!$password)
				return error("Invalid argument: missing password");
			else if (!in_array($domain, $this->email_list_virtual_transports()))
		...
		public function create_mailing_list_backend($list, $password, $email, $domain) {
			...
			$mapping = system('/usr/sbin/postalias -q majordomo+'.$domain.' '.self::POSTFIX_LOCAL_ALIASES_FILE.' > /dev/null', $ret);
			if ($ret === 1) {
			...
			file_put_contents($prefix.'/etc/majordomo-'.$domain.'.cf',
				preg_replace('/^\s*\$whereami.+$/m','$whereami = "'.$domain.'";',file_get_contents($prefix.'/etc/majordomo.cf')));
			...
			file_put_contents($prefix.'/var/lib/majordomo/lists/'.$list.'.config',
				$this->change_configuration_options(array('admin_passwd' => $password,
				                                          'resend_host'    => $domain,
				                                          'restrict_post'  => $list,
				                                          'sender'         => 'owner-'.$list)));
		...
		public function delete_mailing_list($list) {
			...
			if (!preg_match(Regex::MAILING_LIST_NAME,$list))
				return error("Invalid list name");
			else if (!$this->mailing_list_exists($list)) {
			...
			foreach (explode("\n",file_get_contents(self::POSTFIX_LOCAL_ALIASES_FILE)) as $line) {
				if (preg_match('!'.$list.'(?:-outgoing|-request)?\+'.$domain.':!',$line))
					continue;
				else if (!$moreLists && preg_match('!majordomo\+'.$domain.':!',$line))
					continue;
				$lines[] = $line;
			}
		...
		private function _parse_configuration($text) {
			...
			foreach ($matches as $match => $value) {
				if (isset($base[$value[1]])) {
					$base[$value[1]]['value'] = trim(($base[$value[1]]['type'] == text) ? str_replace("END","",$value[2]) : $value[2]);
				}
			}
