	...
	class Php_Module extends Module_Skeleton
	{
		...
		public function list_installed_packages()
		{
			...
			for ($i = 1; $i < $packageSize; $i++) {
				...
				$packages[$packageMatches[1][$i]] = array('is_local' => true, 'version' => trim($packageMatches[2][$i]));
			}
			ksort($packages);
			return $packages;
		}
		...
		public function list_remote_packages()
		{
			...
			if ($status instanceof Exception)
				return $status;
			$pear = array();
			$pearCount = preg_match_all('!^pear/([^\s]+)\s+([^\s]+)\s+([0-9\.]*)\s+(.*)$!m', $status['output'], $pearTmp);
