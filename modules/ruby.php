<?php
    ...
    class Ruby_Module extends Module_Skeleton {
		...
		public function get_gem($gem, $local = false) {
			...
			if (!isset($gems[$gem])) return array();
			return $gems[$gem];
		}

		public function gem_version($gem, $local = false) {
			$info = $this->get_gem($gem, $local);
			if (!empty($info['versions'])) return array_shift($info['versions']);
			return $info['version'];
		}
		...
		public function install_gem($gem, $ver = null) {
			if (!IS_CLI) return $this->query('ruby_install_gem',$gem, $ver);
        ...
        public function list_installed_gems($rubyver = NULL) {
			if (!$rubyver) $rubyver = self::RUBY_VERSION;
			if (!IS_CLI) {
        ...
        public function list_remote_gems() {
			...
			if ($status instanceof Exception)
				return $status;
			$gems = array();
			$gemCount = preg_match_all('!^([^(\n]+) \(([0-9,\. ]+)\)([\s\S]+?)^\s*$\n([\s\S]+?)^$$!m', $status['output'], $gemsTmp);

			for ($i = 1; $i < $gemCount; $i++) {
				$gems[$gemsTmp[1][$i]] = array('versions' => array_map('trim',explode(',',$gemsTmp[2][$i])),
		...
		public function rubygems_version()
		{
			...
			do {
				$line = fgets($fp);
				$tok = strtok(" ", $line);
				if ($tok != "RubyGemsVersion") continue;
				strtok(" "); // =
				$version = trim(strtok(" "),'"\'');
			} while (!feof($fp));
			fclose($fp);
			if (!$version) return error("version.rb is corrupted");
			return $version;
		}
        
        public function uninstall_gem($gem, $ver = null) {
			if (!IS_CLI) return $this->query('ruby_uninstall_gem',$gem, $ver);
			if (!preg_match('!^[a-zA-Z0-9_-]+$!', $gem))
				return error($gem.": invalid gem");
			$proc  = Util_Process_Tee::watch(new Util_Process_Sudo);
			...
			if (!$ver) $flags = '-a';
			else $flags = '-v '.escapeshellarg($ver);
        ...
        public function version($full = false) {
            if (!IS_CLI) return $this->query('ruby_version', $full);
			if (!$this->ruby_exists()) return false;
			$version = Util_Process_Sudo::exec('ruby -e "print RUBY_VERSION"');
			$version = $version['output'];
			if (!$full) return substr($version,0,strrpos($version,'.'));
			return $version;
        }
