<?php
	 ...
	 *  | apnscp esprit                                              |
	 *  +------------------------------------------------------------+
	 *  | Copyright (c) 2005 Apis Networks                           |
	 *  +------------------------------------------------------------+
	...
	class Site_Module extends Module_Support_Auth {
			...
			switch ($type) {
				case null:
					$bw_rs = $this->pgsql->query("SELECT
				 	...
				 	if (!$bw_rs) return array('used' => 0, 'total' => -1);
				 	...
				 	$used_rs = $this->pgsql->query("SELECT
						...
						AND
						ts >= '".$bw_rs->begindate."'");
					return array('used'  => (float)$used_rs->fetch_object()->sum,
		...
		public function get_account_quota() {
			if (!IS_CLI) return $this->query('site_get_account_quota');
        ...
        public function wipe($token = null) {
            ...
            if (!IS_CLI) return $this->query('site_wipe',$token);
