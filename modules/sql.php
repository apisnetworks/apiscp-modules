<?php
    ...
    class Sql_Module extends Module_Support_Sql
    {
        ...
        private function _delete_temp_mysql_user($user)
        {
            ...
            if ($idx !== false) {
                unset($this->_tempUsers['mysql'][$idx]);
            }
            return true;
        }
        ...
        public function delete_mysql_user($user, $host, $cascade = true)
        {
            if ($user == $this->username && !Util_Account_Hooks::is_mode('delete')) {
                ...
                if (!$this->mysql_user_exists($user, $host)) {
        ...
        private function _delete_temp_pgsql_user($user)
        {
            ...
            if ($idx !== false) {
                unset($this->_tempUsers['pgsql'][$idx]);
            }
            return true;
        }
        ...
        public function retrieve_sql_password($type)
        {
            ...
            if ($type == 'mysql') {
                return $this->get_mysql_option('password');
            } // deprecated
            else {
                if ($type == 'pgsql') {
        ...
        public function change_prefix($prefix)
        {
            ...
            $map = \Opcenter\Map::load('mysql.prefixmap');
        ...
        public function create_mysql_database($db, $charset = 'latin1', $collation = 'latin1_general_ci')
        {
            ...
            if (!$this->mysql_collation_valid($collation)) {
                ...
                if (!$this->mysql_collation_compatible($collation, $charset)) {
        ...
        public function edit_mysql_user($user, $host, $opts)
        {
            ...
            if ($mergeopts['max_questions'] < 0 || $mergeopts['max_updates'] < 0) {
                ...
                if (isset($opts['max_user_connections']) && $opts['max_user_connections'] > 10) {
        ...
        public function enabled($svc_name)
        {
            ...
            return $this->get_service_value($svc_name, 'enabled');
        }
        ...
        public function prep_tablespace()
        {
            ...
            if (!file_exists($path)) {
                $this->query("sql_prep_tablespace_backend", $path);
            }
         ...
         * @return bool
         */
        public function prep_tablespace_backend($path)
        {
            if (!is_dir($location)) {
        ...
        private function _create_temp_pgsql_user($db)
        {
            ...
            $maxlen = self::MYSQL_USER_FIELD_SIZE - strlen($prefix);
            if ($maxlen < 1) {
                return error("temp mysql user exceeds field length");
            }
            $chars = array(
                'a',
                'b',
        ...
        public function get_database_size($type, $db)
        {
            ...
            if ($type == "pgsql" || $type == "postgresql") {
                ...
                $resp = $this->query('sql_get_database_size', $type, $db);
                return (int)$resp;
            }
                ...
                if (!in_array($extension, array('gz', 'bz', 'zip', 'none'))) {
                    ...
                    if (intval($span) != $span || intval($preserve) != $preserve) {
                        ...
                        if ($span < 1) {
                            ...
                            if (intval($email) != $email && !preg_match(Regex::EMAIL, $email)) {
        ...
        private function edit_sql_backup($type, $db, $extension, $span, $preserve, $email)
        {
            ...
            if (!in_array($db, $dbs)) {
                ...
                if (!in_array($extension, array('gz', 'bz', 'zip', 'none'))) {
                    ...
                    if (!ctype_digit($span) || !ctype_digit($preserve)) {
                        return error("non-numeric type for day span/preservation amount");
                    } else {
        ...
        public function _delete()
        {
            ...
            foreach (['mysql', 'pgsql'] as $lang) {
                if ($this->sql_enabled($lang) && !$this->uninstallDatabaseService($lang)) {
                    warn("failed to delete %s service from `%s'", $lang, $conf['siteinfo']['domain']);
                }
            }
        }
        ...
        public function _edit_user($userold, $usernew) {
