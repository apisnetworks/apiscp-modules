<?php
    ...
    class Urchin_Module extends Module_Skeleton
    {
        ...
        {
            if (!$profile) {
                $ids = $this->_urchin_driver('get_parameter', 'logfile',
                    array('parameter' => 'cs_rlist', 'name' => $log_name));
                $ret = true;
                foreach ($ids as $id) {
                    $profile = $this->_urchin_driver('get_parameter', 'profile',
                        array('parameter' => 'ct_name', 'recnum' => $id));
            ...
            if ($logmeta['ct_affiliation'] != $this->site) {
                return error("permission denied: `$log_name' cannot remove");
            }
        ...
        {
            $ret = $this->_urchin_driver('get_parameter', 'profile', array(
                'parameter' => 'ct_affiliation',
                'name'      => $mProfile
            ));
        ...
        {
            $log = 'access_log';
            $domain = preg_replace('/[^[a-z0-9\.\*-]]/', '',
                strtolower(is_null($domain) ? $this->get_service_value('siteinfo', 'domain') : $domain));
            ...
            if ($this->profile_exists($domain)) {
                ...
                if ($this->get_profile_count() > $profile_limit) {
            ...
            $u_rec = $u_rec[1];
            $p_rec = $this->_urchin_driver('add', 'profile', array(
                'name'              => $domain,
            ...
            $l1_rec = $this->_urchin_driver('add', 'logfile', array(
                'name'           => '"' . $domain . '-' . $log . '"',
                'ct_affiliation' => $this->site,
                'cs_logformat'   => 'auto',
            ...
            $l2_rec = $this->_urchin_driver('add', 'logfile', array(
                'name'           => '"' . $domain . '-' . $log . '.[1-4]"',
                'ct_affiliation' => $this->site,
                'cs_logformat'   => 'auto',
            ...
            $t_rec = $this->_urchin_driver('add', 'task', array(
                'name'           => $domain,
                'ct_affiliation' => $this->site,
                'cr_frequency'   => '5',
                'cr_minute'      => '0',
            ...
            $this->_urchin_driver('set_parameter', null, array(
                'recnum'   => $p_rec,
                'cs_ulist' => '!' . $u_rec . '!'
            ));
            $this->_urchin_driver('set_parameter', null, array(
        ...
        {
            $ret = $this->_urchin_driver_bool('get_parameter', 'affiliation',
                array('parameter' => 'ct_contact', 'name' => $this->site));
            return $ret;
        }
        ...
        public function create_profile_backend($mProfile, $mLogs)
        {
            ...
            (file_exists(self::URCHIN_BASE . '/urchin/data/reports/' . $mProfile) ||
                symlink($this->domain_fs_path() . '/var/lib/urchin/' . $mProfile,
                    self::URCHIN_BASE . '/urchin/data/reports/' . $mProfile));
        ...
        {
            if (!IS_CLI) {
                return $this->query('urchin_locate_profile');
            }
            $installations = array();
            $files = str_replace('//', '/', glob(self::URCHIN_BASE . '/urchin*/data/reports/' . $this->domain));
            foreach ($files as $installation) {
                $installations[] = substr($installation, strlen(self::URCHIN_BASE),
                    strpos(substr($installation, strlen(self::URCHIN_BASE)), '/'));
            }
        ...
        public function change_password($mValue, $mUser = null)
        {
            ...
            return $this->_urchin_driver_bool('set_parameter', 'user',
                array('name' => $mUser, 'ct_password' => $mValue));
        }

        public function user_is_affiliated($mUser)
        {
            $ret = $this->_urchin_driver_bool('get_parameter', 'user', array(
                'parameter' => 'ct_affiliation',
                'name'      => $mUser
            ));
        ...
        public function get_profiles()
        {
            ...
            if (is_array($cacheprof)) {
                return $cacheprof;
            }
            $user = $this->_urchin_driver('get', 'user', array('recnum' => $id));
            if (!isset($user['cs_rlist'][0])) {
                return $profiles;
            }
            foreach ($user['cs_rlist'] as $profileid) {
                $drv = $this->_urchin_driver('get_parameter', 'profile',
                    array('parameter' => 'ct_name', 'recnum' => $profileid));
                // orphan profile
                if ($drv == '-1') {
                    continue;
                }
        ...
        {
            if (!IS_CLI) {
                return $this->query('urchin_get_urchin_disk_usage');
            }
            $size = 0;
        ...
        {
            if (!IS_CLI) {
                return $this->query('urchin_delete_profile', $profile);
            }
            ...
            foreach ($profilemeta['cs_ulist'] as $user) {
                $profiles = $this->_urchin_driver('get_parameter', 'profile',
                    array('parameter' => 'cs_rlist', 'recnum' => $user));
                $key = array_search($id, $profiles);
                if ($key === false) {
                    continue;
                }
                unset($profiles[$key]);
                $this->_urchin_driver('set_parameter', 'profile',
                    array('recnum' => $user, 'cs_rlist' => '!' . join('!', $profiles) . '!'));
            }
            ...
            if ($mAction == 'add') {
                ...
                if ($mAction == 'get') {
                    if (!$output['success']) {
                        ...
                        return $this->_parse(trim($output['output']));
                    }
                } else {
                    if ($mAction == 'get_parameter') {
                        if (!$output['success']) {
                            return null;
                        }
                        $resp = rtrim($output['output']);
                        if (isset($resp[0]) && $resp[0] == "!") {
                            $resp = explode("!", substr($resp, 1, -1));
                        }
                        return $resp;
                    } else {
                        if ($mAction == 'delete') {
        ...
        private function _getProfileID($profile)
        {
            ...
            if (!$this->profile_exists($profile)) {
        ...
        {
            return
                $this->_urchin_driver_bool('add', 'affiliation', array(
                    'name'               => $this->site,
